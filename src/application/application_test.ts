...
test('listenEvent() should listen for MESG events', function (t) {
  ...
  application.listenEvent('1', { eventKey: '2' });
  t.ok(spy.calledOnce);
...
test('listenResult() should listen for MESG results', function (t) {
  ...
  application.listenResult('1', { taskKey: '2', outputKey: '3', tags: ['4', '5'] });
  t.ok(spy.calledOnce);
...
test('executeTask() should execute a MESG task', function (t) {
  ...
  application.executeTask('1', '2', {"3": "b"}, ['4', '5'])
  ...
  cb(null, <ExecuteTaskReply>{ getExecutionid() { return '6'} });
  spy.restore();
});
